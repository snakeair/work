<template>
  <div>
  <div class="reg-pages">
    <div class="z10">
    <h1 class="title"><img class="logo" src="@/assets/images/img-06.png" alt=""></h1>
    <div class="banner">
      <h3 class="money" ><em class="icon">￥</em>200,000</h3>
      <p class="info">年化利率7.2%-24% <em>/</em> 循环额度 <em>/</em> 次日可还</p>
    </div>
    <div class="form-box" >
      <form name="login" id="login-form" @submit.native.prevent >
        <div class="form-list">
          <input class="input" type="text" maxlength="11" v-model="loginForm.mobile" placeholder="请输入您的联系电话" >
        </div>
        <div class="form-list form-code ">
          <input class="input code" type="text" maxlength="11" v-model="loginForm.code" placeholder="请输入短信验证码">
          <div btn v-show="getCode.show"  class="get-code-btn" @click="getCodeFn">获取验证码</div>
          <div class="wait-code" v-show="!getCode.show"><em>{{getCode.timer}}s</em> 后重新获取</div>
          <p class="error" v-show="error.type" >{{error.msg}}</p>
        </div>
        <button class="form-btn"  type="submit"  @click="submitFn()">登录</button>
      </form>
      <div class="pact">
        <span class="checkbox" @click="checkboxFn" >
          <img v-show="!regCheck" class="img" src="@/assets/images/img-08.png" alt="">
          <img v-show="regCheck" class="img" src="@/assets/images/img-088.png" alt="">
        </span>
        我已阅读并同意<router-link to="/pact/reg" >《趣融分期注册协议》</router-link></div>
    </div>
    <footer class="footer">
      <p class="footer-p">贷款有风险，借款需谨慎</p>
      <p class="footer-p">贷款资金由银行和持有贷款业务资格文件的正规小额贷款公司提供</p>
      <p class="footer-p">重庆小康小额贷款有限公司</p>
      <p class="footer-p">渝公网安备 50019002501472号 渝ICP备18000384号-1</p>
    </footer>
    </div>
  </div> 

  <!-- <div class="cloud" v-show="layerShow != ''"></div>
  <div class="layer layer-address" v-show="layerShow == 'addrShow'">
    <h1 class="layer-title"><em class="icon"><img class="img" src="@/assets/images/img-10.png" alt=""></em>您在哪个城市工作</h1>
    <h3 class="two-title">机构仅办理当地工作人士申请及咨询</h3>
    <section class="address-select">
      <div class="addr-box">北京</div>
      <div class="addr-box">上海</div>
      <router-link to="/address" class="addr-box">其他城市</router-link>
    </section>
    <div class="layer-close" @click="closeFn"><em class="iconfont icon-guanbi1"></em></div>
  </div> -->
</div>
</template>

<script setup lang="ts">
import {phoneFn, isNoneFn} from '@/assets/js/util'
import { $get, $post } from '@/assets/js/axios'
import {indexStore} from '@/store/index'
import $message from '@/cart/message';
import cookie from 'js-cookie';
import { isLoginFn } from '@/assets/js/mixins'
const route = useRoute()
const router = useRouter()

const store = indexStore()

interface tsToken {
  Time: string,
  Val: string,
  ID: string
}



// 登陆相关
const error = reactive<tsError>({
  msg: '',
  type: false,
})

const loginForm = reactive<tsLoginForm>({
  code: '',
  mobile: ''
})
const getCode = reactive<tsGetCode>({
  timer: 60,
  show: true,
})

let errPhone = '请输入正确的手机号码'

// 获取验证码
const getCodeFn = () => {
  if (!phoneFn(loginForm.mobile)) {
    error.type = true;
    error.msg = errPhone;
    return false;
  } else {
    error.type = false;
  }
  let url = `/api.php/core/tool/getcode?client=h5&package=pc&ver=v1.1&Mobile=${loginForm.mobile}&type=3`
  
  $get(url, {}).then( (res:any) => {
    getCode.show = false;
    setInterval(() => {
      if (getCode.timer === 1) {
        getCode.timer = 60;
        getCode.show = true;
      } else {
        getCode.timer--
      }
    }, 1000)
  })
}

// 提交表单 注册
const submitFn = () => {
  let bl = phoneFn(loginForm.mobile)
  if (!bl) {
    error.type = true;
    error.msg = errPhone;
    return false;
  } else {
    error.type = false;
  }
  if (!regCheck.value) {
    error.type = true;
    error.msg = '请同意注册协议'
    return false;
  } else {
    error.type = false;
  }
  let puser = route.query.source
  let url = `/api.php/center/Register/reg`
  let data:tsAny = {
    Mobile: loginForm.mobile,
    Code: loginForm.code,
    puser: puser ? puser : store.puser,
    client: 'pc'
  }
  $post(url, data).then( (res:any) => {
    console.log(res.rsult);
    
    if (res.result === 1 || res.result === 3) {
      toLoginFn();
    } else {
      error.type = true;
      error.msg = res.message
    }
  })
}

const tokenData = ref<tsToken>({
  Time: '',
  Val: '',
  ID: ''
})
const toLoginFn = () => {
  let url = `/api.php/core/tool/timestamp?client=pc&package=h5&ver=v1.1`
  $get(url, {}).then((res: any) => {
    if (res.result == 1) {
      console.log('get token');
      tokenData.value = res.data;
      loginFn()
    }
  })
}

const loginFn = () => {
  console.log('login');
  let url = `/api.php/center/member/login`
  let data = {
    ticksid: tokenData.value.ID,
    client: 'h5',
    package: 'pc',
    ver: 'v1.1',
    ticks: tokenData.value.Val,
    mobile: loginForm.mobile,
    code: loginForm.code,
    loginType: 1
  }
  $get(url, data).then( (res:any) => {
    if(res.result == 1) {
      cookie.set('token', res.data.Token, { expires : 0.04})
      cookie.set('id', res.data.ID, { expires: 0.04 })
      cookie.set('info', res.data.is_info, { expires: 0.04 })
      isLoginFn();
      router.push({
        path: '/address'
      })
    } else {
      $message({type: 'error', msg: res.message})
    }
  })
}
// 登陆相关  -- end


// 协议
const regCheck = ref<boolean>(false)
const checkboxFn = () => {
  regCheck.value = !regCheck.value
}



// 统计代码
const channelFn = () => {
  let url = `/api.php/center/register/channel`
  let puser = route.query.source
  let data:tsAny = {
    puser: puser ? puser : store.puser,
  }
  $post(url, data).then((res:any) => {})
}
channelFn();


// 判断登陆
const isLoginFn = () => {
  let token = cookie.get('token');
  if (token) {
    router.push({
      path: '/'
    })
  }
}
isLoginFn();


</script>

<style scoped lang="scss">
@media only screen and (min-width: 800px) {
  .reg-pages{
    width: 500px;
    margin: 0 auto 0
  }
}
.black-bg{
    position: absolute;
    z-index: 1; width: 100vw; height:size(230);
    .bg{
      display: block;
      width:100%; height:100%;
    }
  }
.reg-pages{
  min-height: 100vh; margin:0 auto 0;
  background: $bg url("@/assets/images/img-07.png") no-repeat top center; background-size: 100%;
  
  .z10{
    position: relative;
    z-index: 10;
  }
  .title{
    box-sizing: border-box;
    height:size(110); padding-left: size(30); padding-top: size(25);
    .logo{
      width:size(433); height:size(42);
    }
  }
  .banner{
    box-sizing: border-box;
    width:size(690); height:size(162); margin:0 auto 0; padding-left: size(60); padding-top: size(40);
    background: url('@/assets/images/img-05.png') no-repeat; background-size: 100%; 
    .money{
      font-size: size(55); line-height: 1; 
      color:$white;
      .icon{
        font-size: size(43);
      }
    }
    .info{
      font-size: size(24); opacity: 0.5; padding-left: size(10); margin-top: size(10); line-height: 1;
      color:$white;
      em{
        margin: 0 size(12);
      }
    }
  }
  .form-box{
    box-sizing: border-box;
    width: size(690); margin:0 auto 0; padding-top: size(40); margin-top: size(30);  padding-left: size(30); padding-right: size(30); padding-bottom: size(40); border-radius: 10px; margin-bottom: size(60);
    background-color: #fff;
    .form-list{
      margin-bottom: size(30);
      &:nth-of-type(2){
        margin-bottom: size(70);
      }
    }
    .input{
      box-sizing: border-box; 
      width:100%;  height:size(88); line-height: size(88); padding-left: size(30);  width:100%; font-size: size(28); 
      background-color: rgba($color: #fff, $alpha: 0); color:#666; border-bottom: 1px solid $border;
      transition: all .5s;
      &:focus{
        border-color:$web1;
      }
    }
    .error{
      position: absolute;
      font-size: size(20); line-height: 1; margin-top: size(10);
      color:$red;
    }
  .form-btn{
    text-align: center;
    width:100%; height:size(88); line-height: size(88); border-radius: size(44); font-size: size(34);
    background-color: $web3;  color:#fff; background-image: linear-gradient(94deg, #fd751c 0%, #f34638 100%);
    
  }
  
  .form-code{
    position: relative;
  }
  .get-code-btn{
    text-align: center;  position: absolute;
    font-size: size(28); width:size(220); height:size(88); line-height: size(88);  right:0; bottom:0;
    color:$web1;
      &:before{
      content: '';
      display: block; position: absolute;
      width:1px; height:20px; margin-top: 12px;
      background-color: $border;
    }
  }
  .wait-code{
    @extend .get-code-btn;
    color:#888888;
    em{
      color:#f34837;
    }
  }
}
  .pact{
    vertical-align: middle; 
    margin-top: 10px; font-size: size(22); 
    .checkbox{
      vertical-align: middle;  display: inline-block;
      margin-top: -2px; margin-right: 4px; width:size(25); height:size(25);
      .img{
        display: block;
        width:100%;
      }
    }
    a{
      font-size: size(22);
      color:$orange;
    }
  }
  .footer{
  text-align: center;
  .footer-p{
    font-size: size(20); line-height: size(26); margin-bottom: size(15); line-height: 1;
    color:#aaa;
    &:first-of-type{
      font-size: size(26); margin-bottom: size(25);
    }
  }
}
}

.layer-address{
  width:size(690); height:size(536);
  .layer-title{
    text-align: center; vertical-align: middle; font-weight: bold;
    font-size: size(35); line-height: size(40); height:size(40); margin-top: size(40); margin-bottom: size(15);
    color:$orange;
    .icon{
      display: inline-block;  vertical-align: middle;
      width:size(27); height:size(35); margin-right: 5px; margin-top: -2px;
      .img{
        display: block;
        width:100%;
      }
    }
  }
  .two-title{
    text-align: center;
    font-size: size(24); line-height: 1;
    color:#888888;
  }
  .address-select{
    margin-top: size(40);  padding-bottom: size(20);
    .addr-box{
      text-align: center; display: block;
      width:size(320); height:size(94); line-height: size(94);  border-radius: 8px; font-size: size(28); margin:0 auto  size(30);
      color:#fff; background-image: linear-gradient(94deg, #fd751c 0%,  #f34638 100%);
    }
  }
}
</style>